...
Linux:
```bash
curl -L https://github.com/rcoh/angle-grinder/releases/download/v0.7.6/angle_grinder-v0.7.6-x86_64-unknown-linux-musl.tar.gz \
  | tar Ozxf - \
...
OS X:
```bash
curl -L https://github.com/rcoh/angle-grinder/releases/download/v0.7.6/angle_grinder-v0.7.6-x86_64-apple-darwin.tar.gz \
  | tar Ozxf - \
  | sudo tee /usr/local/bin/agrind > /dev/null && sudo chmod +x /usr/local/bin/agrind
```
